...
![wumpus-example](wumpus-world.png)
...
At the beginning we assume that the agent starts in the bottom left square (0,0) facing up (N). To climb out of the cave the agent must go back to the starting square. The game ends when the agent climbs out, dies, or after a fixed number of actions.

At the end of the game the overall score is calculated as the sum of:

- +1000 for leaving cave with gold (just grabbing it doesn't grant the prize)
