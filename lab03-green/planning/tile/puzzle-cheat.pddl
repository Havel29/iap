...
(define (domain sliding-tile)
  ...
  (:action remove-tile
      ...
      :precondition (and (IS-TILE ?tile)
                      ...
                      (tile-at ?tile ?row ?col)
                      (not(tile-removed)))
      :effect (and (is-blank ?row ?col)
                  (is-removed ?tile) ;remove a certain tile
                  (not (tile-at ?tile ?row ?col)) ; the tile is no more at the row,col position
                  (tile-removed))) ;set tile-removed to true, no other tile can be removed unless it is reinserted
  ...
  (:action reinsert-tile
      ...
      :precondition (and (IS-TILE ?tile)
                      ...
                      (IS-COLUMN ?col)
                      (is-removed ?tile)
                      (not (tile-at ?tile ?row ?col))
                      (is-blank ?row ?col)
                      (tile-removed)
                      )
      :effect (and (not(is-blank ?row ?col))
                  (not(is-removed ?tile))
                  (tile-at ?tile ?row ?col) ;insert tile previously removed
                  (not(tile-removed)))) ;allow to remove other tile
